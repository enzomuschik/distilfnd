...
def main():
    ...
    if col2.button(label="PREDICT POST"):
        ...
        df_pred = pd.DataFrame(
            # Iteration through probability values of prediction tensor via list comprehension
            # and assignment of corresponding values per class to probabilities key
            # [item for item in prediction_probs.detach().numpy()][0],
            prediction_probs.detach().numpy(),
            columns=CLASS_NAMES
        )
        # Loading bar plot images and assigning it to plot variable (local version)
        ...
        # First line for web-hosted application, second line for locally hosted
        col3.bar_chart(df_pred)
        #col3.image(image=plot, use_column_width=True, clamp=True, channels="RGB") (localhost version)
